...
class engine:
    ...
    ## @@@@@@@@@@
    def singular_noun(self, text: str, count: Optional[Union[int, str]] = None, gender: Optional[str] = None,) -> Union[str, bool]:
        """
        ...
        if not word:
            return text
        sing = self._sinoun(word, count=count, gender=gender) ### @@@@ need to write this func
    ...
    def _sinoun( self, word: str, count: Optional[Union[str, int]] = None, gender: Optional[str] = None, ) -> Union[str, bool]:
        #@@@@ we don't care about count or gender
        count = self.get_count(count)
        ...
        # SET THE GENDER
        try:
            if gender is None:  gender = self.thegender
            elif gender not in singular_pronoun_genders:
                raise BadGenderError
        except (TypeError, IndexError): raise BadGenderError

        # HANDLE USER-DEFINED NOUNS @@ we don't care about this
        value = self.ud_match(word, self.si_sb_user_defined)
        if value is not None:           return value
        # @@@@ start here: we want to use classical_dict
        # HANDLE EMPTY WORD, SINGULAR COUNT AND UNINFLECTED PLURALS
        if word == "":                  return word
        if word in si_sb_ois_oi_case:   return word[:-1]

        words = Words(word)

        if words.last.lower() in pl_sb_uninflected_complete:    return word
        if word in pl_sb_uninflected_caps:      return word
        ...
        if words.last.lower() in pl_sb_C_us_us: return word

        # HANDLE COMPOUNDS ("Governor General", "mother-in-law", "aide-de-camp", ETC.)
        mo = PL_SB_POSTFIX_ADJ_STEMS_RE.search(word)
        if mo and mo.group(2) != "":    return f"{self._sinoun(mo.group(1), 1, gender=gender)}{mo.group(2)}"
        ...
        if len(space_split) >= 3:
            for numword in range(1, len(space_split) - 1):
                if space_split[numword] in pl_prep_list_da:
                    ...
                    # typing.Literal in 3.8 will likely help us avoid these, but for now, special case
        ...
        try: return get_si_pron("nom", words.lower, gender)
        except KeyError: pass

        try: return get_si_pron("acc", words.lower, gender)
        except KeyError: pass
        ...
        if words.lower[-5:] == "quies":     return word[:-3] + "y"
        if words.lower[-7:] == "persons":   return word[:-1]
        if words.lower[-6:] == "people":    return word[:-4] + "rson"

        # HANDLE FAMILIES OF IRREGULAR PLURALS
        if words.lower[-4:] == "mans":
            for k, v in si_sb_U_man_mans_bysize.items():
                if words.lower[-k:] in v:   return word[:-1]
            for k, v in si_sb_U_man_mans_caps_bysize.items():
                if word[-k:] in v:      return word[:-1]
        if words.lower[-3:] == "men":   return word[:-3] + "man"
        if words.lower[-4:] == "mice":  return word[:-4] + "mouse"
        if words.lower[-4:] == "lice":  return word[:-4] + "louse"
        if words.lower[-5:] == "geese": return word[:-5] + "goose"
        if words.lower[-5:] == "teeth": return word[:-5] + "tooth"
        if words.lower[-4:] == "feet":  return word[:-4] + "foot"
        if words.lower == "dice":       return "die"

        # HANDLE UNASSIMILATED IMPORTS
        if words.lower[-4:] == "ceps":  return word
        if words.lower[-3:] == "zoa":   return word[:-1] + "on"
        ...
        # HANDLE INCOMPLETELY ASSIMILATED IMPORTS
        if self.classical_dict["ancient"]:
            if words.lower[-6:] == "trices":            return word[:-3] + "x"
            if words.lower[-4:] in ("eaux", "ieux"):    return word[:-1]
            ...
            for lastlet, d, class_numend, post in (
                ...
                if words.lower[-1] == lastlet:  # this test to add speed
                    for k, v in d.items():
                        if words.lower[-k:] in v:   return word[:class_numend] + post
        ...
        if words.last.lower() in si_sb_singular_s_complete: return word[:-2]

        for k, v in si_sb_singular_s_bysize.items():
            if words.lower[-k:] in v:   return word[:-2]

        if words.lower[-4:] == "eses" and word[0] == word[0].upper():
            return word[:-2]

        if words.last.lower() in si_sb_z_zes:   return word[:-2]
        if words.last.lower() in si_sb_zzes_zz: return word[:-2]
        if words.lower[-4:] == "zzes":          return word[:-3]

        if word in si_sb_ches_che_case or words.last.lower() in si_sb_ches_che:
            return word[:-1]

        if words.lower[-4:] in ("ches", "shes"):return word[:-2]
        if words.last.lower() in si_sb_xes_xe:  return word[:-1]
        if words.lower[-3:] == "xes":           return word[:-2]
        ...
        if words.lower[-3:] == "ves":
            if words.lower[-5:-3] in ("el", "al", "ol"):    return word[:-3] + "f"
            if words.lower[-5:-3] == "ea" and word[-6:-5] != "d":   return word[:-3] + "f"
            if words.lower[-5:-3] in ("ni", "li", "wi"):    return word[:-3] + "fe"
            if words.lower[-5:-3] == "ar":                  return word[:-3] + "f"
        ...
        if words.lower[-3:] == "ies":   return word[:-3] + "y"

        # HANDLE ...o
        if words.lower[-2:] == "os":
            if words.last.lower() in si_sb_U_o_os_complete: return word[:-1]
        ...
        if words.lower[-3:] == "oes":   return word[:-2]

        # UNASSIMILATED IMPORTS FINAL RULE
        if word in si_sb_es_is:         return word[:-2] + "is"

        # OTHERWISE JUST REMOVE ...s
        if words.lower[-1] == "s":      return word[:-1]
