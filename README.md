...
Build the shared Proteus engine from the project root:
...
```bash
# Build the test executable via the LocalBuild config
codeDog Proteus.Lib.dog
...
# Compile and run the C++ test harness separately from the build directory
codeDog -b TestProteus
```
